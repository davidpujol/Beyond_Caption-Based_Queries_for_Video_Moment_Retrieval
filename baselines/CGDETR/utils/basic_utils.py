...
def load_jsonl(filename):
    with open(filename, "r") as f:
        return [
            json.loads(l)
            for l in f
            if l.strip()  # skip empty or whitespace-only lines
        ]
...
def l2_normalize_np_array(np_array, eps=1e-5, axis=-1):
    """np_array: np.ndarray, (*, D), where the last dim will be normalized"""
    return np_array / (np.linalg.norm(np_array, axis=axis, keepdims=True) + eps)
