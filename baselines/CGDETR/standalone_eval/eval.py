...
def compute_mr_r1(
    ...
    # Compute average only over thresholds >= 0.3
...
def compute_mr_recall_at_n(
    ...
    pred_qid2windows = {d["qid"]: [w[:2] for w in d["pred_relevant_windows"]] for d in submission}
    ...
    for qid in qids:
        ...
        # For each GT, check if any prediction matches it above threshold
        for thd in iou_thds:
...
def eval_moment_retrieval(submission, ground_truth, verbose=True):
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        if len(_ground_truth) == 0:
            ...
            ret_metrics[name] = {"MR-mAP": dummy_dict, "MR-R1": dummy_dict, "MR-RN": dummy_dict}
        else:
