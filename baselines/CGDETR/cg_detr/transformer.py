...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        decoder_layer = TransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before, keep_query_pos=keep_query_pos, args=args, rm_self_attn_decoder=args.rm_self_attn_decoder)
        decoder_norm = nn.LayerNorm(d_model)
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                          return_intermediate=return_intermediate_dec,
                                          d_model=d_model, query_dim=query_dim, keep_query_pos=keep_query_pos, query_scale_type=query_scale_type,
                                          modulate_t_attn=modulate_t_attn,
                                          bbox_embed_diff_each_layer=bbox_embed_diff_each_layer, args=args)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 ...
                 rm_self_attn_decoder=False, args=None):
        super().__init__()
        self.args = args
    ...
    def forward(self, tgt, memory,
        ...
        if not self.rm_self_attn_decoder:
            ...
            # ------------------------
            # Added by me: Compute random masks for the queries
            if tgt_mask is None and self.args.sa_drop_rate > 0 and self.training:
                num_queries, bs, _ = tgt.shape
                drop_rate = self.args.sa_drop_rate
                num_drop = int(round(drop_rate * num_queries))

                # (num_queries, num_queries) mask: True = blocked attention
                attn_mask = torch.zeros(num_queries, num_queries, dtype=torch.bool, device=tgt.device)

                if num_drop > 0:
                    # For now we assume that every batch gets the same random drop.
                    
                    # Randomly select queries to drop for this sample
                    drop_idx = torch.randperm(num_queries, device=tgt.device)[:num_drop]

                    # Block them as queries (they cannot attend to anybody)
                    attn_mask[drop_idx, :] = True
                    
                    # Block the self-attention for dropped queries (so no one can attend to them)
                    attn_mask[:, drop_idx] = True
                    
                    # To avoid getting nans for the blocked ones, we allow them to attend to themselves
                    attn_mask[drop_idx, drop_idx] = False
                    
            else:
                attn_mask = tgt_mask
            
            # Default case: Normal self attention
            tgt2, weights = self.self_attn(q, k, value=v, attn_mask=attn_mask,
