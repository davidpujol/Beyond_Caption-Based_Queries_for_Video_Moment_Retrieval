...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir,
        ...
        # data
        self.load_data()    # The self.data is now accessible
            ...
            for file_name in os.listdir(_feat_dir):
                if file_name.endswith('.npz'):
                    ...
                    try:
                        ...
                        print("error loading visual_feats_interm_pooled, trying visual_feats_pooled", e)
                        durations[vid] = 0
    ...
    def load_data(self):
        ...
        for i, data in enumerate(datalist):
            ...
            if self.dset_name == 'hd_epic' or self.dset_name == 'yc2':
                ...
                stride = 500  # frames, for sliding window. Adjust as desired (max_len * 0.66) 
                ...
                # ---------------------
                if cut_mode == 'single':
                    # One clip covering all instances (as in your original code). Not really used for this dataset
                    ...
                    for st_f, ed_f in zip(rel_sts, rel_eds):
                        ...
                        if overlap_st < overlap_ed:
                            ...
                            if 'segs_to_evaluate' in data:
                                segs_to_eval.append(data['segs_to_evaluate'][i])
                            else:
                    ...
                    for mw_st, mw_ed in merged_windows:
                        ...
                        for i, (st_f, ed_f) in enumerate(zip(rel_sts, rel_eds)):
                    ...
                    for clip_st in start_positions:
                        ...
                        # Get all the GT that are within this clip (fully or partially). For merged we already filter all the possible relevant_windows, but we may want to consider them independently as independent queries. In this case we need to use the merged windows by qid.
                        rel_windows_clip = []
                ...
                # Load the segments_to_evaluate if present (this applies to datasets like ActivityNet-Captions)
                if 'segs_to_evaluate' in data:
                    data['segs_to_eval'] = data['segs_to_evaluate']
                else:
                    data['segs_to_eval'] = [True] * len(data.get('relevant_windows', [])) if self.load_labels else []
        ...
        # Set te new_data as the self.data parameter
        self.data = new_data
    ...
    def __getitem__(self, index):
        ...
        if self.dset_name == 'hd_epic' or self.dset_name == 'yc2':
            ...
            if len(relevant_windows) == 0:
                ...
                model_inputs["span_labels"] = torch.tensor([]) #torch.tensor([[0, 0]])  # Dummy span label
    ...
    def _get_query_feat_by_qid(self, qid, meta=None):
        ...
        # youtube-hl
            ...
            q_feat = pickle.load(open(q_feat_path, 'rb'))[n_paragraph]
            
            if len(q_feat.shape) == 3:
                # In some cases we get 1xlength x F
                q_feat = q_feat[0]
