...
def expand_env_vars(path):
    ...
    Replaces ${VAR_NAME} and $VAR_NAME with their environment variable values.
    Also expands ~ to user home directory.
    
    Args:
        path: String path that may contain environment variables
    ...
    # Expand $VAR style variables (more conservative - only alphanumeric and underscore)
    path = re.sub(r'\$([A-Za-z_][A-Za-z0-9_]*)', lambda m: os.environ.get(m.group(1), m.group(0)), path)
...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--suffix", type=str, default='',
                    help="this is the suffix that will be added to all the submissions names")
        parser.add_argument("--dset_name", type=str, choices=["hl", 'tvsum', 'charadesSTA', 'tacos', 'nlq','youtube_uni', 'hd_epic', 'synopground', 'yc2'])
        ...
        parser.add_argument("--max_windows", type=int, default=-1) #, default=5)
        ...
        parser.add_argument("--t_feat_dir_eval", type=str, help="text/query feature evaluation dir")
        ...
        parser.add_argument("--max_before_nms", type=int, default=20)
        parser.add_argument("--max_after_nms", type=int, default=20)
        parser.add_argument("--conf_thd", type=float, default=0.0, help="only keep windows with conf >= conf_thd")
        parser.add_argument("--nms_thd", type=float, default=0.4,
        ...
        # ----------
        # Added
        parser.add_argument("--qd_rate", type=float, default=0.0, help="This defines the rate of learnable queries that should be randomly dropped altogether")
        parser.add_argument("--sa_drop_rate", type=float, default=0.0)
        parser.add_argument("--shared_attn_msk", type=bool, default=False)
        parser.add_argument("--rm_self_attn_decoder", type=str2bool, default=False)
        # ----------
    ...
    def parse(self, a_feat_dir=None):
        ...
        if isinstance(self, TestOptions):
            ...
            for arg in saved_options:  # use saved options to overwrite all BaseOptions args.
                if arg not in ["results_root", "num_workers", "nms_thd", "conf_thd", "debug",  # "max_before_nms", "max_after_nms"
                               "max_pred_l", "min_pred_l",
                               "resume", "resume_all", "no_sort_results", 't_feat_dir', 'v_feat_dirs', 'eval_path', 'suffix']:
                    value = saved_options[arg]
                    # Expand environment variables in path-like arguments
            ...
            if opt.exp_name is None:
                raise ValueError("--exp_name is required for at a training option!")

            opt.results_dir = os.path.join(opt.results_root, "-".join([opt.dset_name, opt.exp_name]))
            
            mkdirp(opt.results_dir)
            save_fns = ['cg_detr/model.py']
