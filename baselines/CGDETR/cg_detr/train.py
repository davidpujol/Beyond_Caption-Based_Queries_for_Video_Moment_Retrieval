...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        # Optional: Pass the model with an indicator of the total progress throught the training
        training_progress = (epoch_i / opt.n_epoch)

        outputs = model(**model_inputs, targets=targets, training_progress=training_progress)
        ...
        # Logging (use the unscaled loss for monitoring)
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if opt.eval_path is not None and (epoch_i + 1) % eval_epoch_interval == 0:
            ...
            if opt.dset_name in ['hl', 'hd_epic', 'yc2']:
                if 'MR-full-mAP' in metrics["brief"]:
                    stop_score = metrics["brief"]["MR-full-mAP"]
                if 'MR-full-mult_aware-mAP' in metrics["brief"]:
                    stop_score = metrics["brief"]["MR-full-mult_aware-mAP"]
            else:
...
def start_training():
    ...
    train_dataset_config["data_path"] = opt.train_path
    
    if opt.dset_name in ['hl', 'synopgroun']:
        train_dataset_config["cuts_type"] = 'single'  # In this case, we just treat the entire video as a single instance/cut.
    elif opt.dset_name == 'hd_epic' or opt.dset_name == 'yc2':
        # TODO: This can be trained on the multi cases or on the sliding window cases.
        train_dataset_config["cuts_type"] = 'multi'   # The training of HD-EPIC requires multiple cuts per video (as long as they include all the GT spans)
        
    train_dataset = StartEndDataset(**train_dataset_config)


    eval_dataset_config = train_dataset_config.copy()
    if opt.eval_path is not None:
        eval_dataset_config["data_path"] = opt.eval_path
        eval_dataset_config["txt_drop_ratio"] = 0
        eval_dataset_config["q_feat_dir"] = opt.t_feat_dir_eval
        
        
        # Optionally we can do the sliding window evaluation
        if opt.dset_name == 'hd_epic' or opt.dset_name == 'yc2':
            # In this case we split the video into overlapping windows using sliding windows
            # This way we can evaluate the entire video, thus not making assumptions on a subregion where the GT spans are located.
            eval_dataset_config["cuts_type"] = 'sliding' 
            eval_dataset_config['is_training']=False
