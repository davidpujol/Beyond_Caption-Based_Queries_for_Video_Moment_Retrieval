...
from cg_detr.span_utils import generalized_temporal_iou, span_cxw_to_xx, temporal_iou
...
import numpy as np
from cg_detr.matchers.matcher import build_matcher
...
class CGDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        self.class_embed = nn.Linear(hidden_dim, 2)  # 1: foreground, 1: background
    ...
    # -----------------
    # Added by me to optionally mask out some of the queries
    def dropout_queries(self, q, drop_rate=0.0, training_progress=0.0):
        """
        Drop exactly (drop_rate * Q) queries.
        q: Tensor [Q, F] (learned query embeddings)
        
        Returns:
            kept_q: [Q_kept, F] subset of queries
            keep_mask: [Q] boolean mask, True = kept
        """
        
        drop_rate = self.args.qd_rate
        
        Q = q.shape[0]
        if drop_rate <= 0.0 or not self.training:
            keep_mask = torch.ones(Q, dtype=torch.bool, device=q.device)
            return q, keep_mask

        # Number of queries to drop
        num_drop = int(round(drop_rate * Q))
        num_keep = Q - num_drop

        # Randomly choose which queries to keep
        perm = torch.randperm(Q, device=q.device)
        keep_idx = perm[:num_keep]
        keep_mask = torch.zeros(Q, dtype=torch.bool, device=q.device)
        keep_mask[keep_idx] = True        
        
        # This is the zeroing out alternative
        # Broadcast mask to (B, Q, D)
        keep_mask_expanded = keep_mask.view(Q, 1).to(q.dtype)  # (Q, 1)
        kept_q = q * keep_mask_expanded                        # (Q, F)
        
        return kept_q, keep_mask


    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, vid, qid, src_aud=None, src_aud_mask=None, targets=None, training_progress=0.0):
        ...
        # ----------------
        # Before anything, mask out some of the learnable queries
        # Note that this removes it altogether. So this would go from shape (n_queries,2) to (n_queries*drop_rate, 2)
        query_embed, _ = self.dropout_queries(self.query_embed.weight, training_progress=training_progress)
        # ----------------
        ...
        if targets is not None: ## train
            ...
            hs, reference, memory, memory_global, attn_weights, memory_moment, nmmemory_moment, mmemory_frames, nmmemory_frames = self.transformer(src, ~mask, query_embed, pos, video_length=video_length, moment_idx=targets["relevant_clips"], msrc=msrc, mpos=mpos, mmask=~mmask, nmsrc=nmsrc, nmpos=nmpos, nmmask=~nmmask,
            ...
            hs, reference, memory, memory_global, attn_weights, memory_moment, nmmemory_moment, mmemory_frames, nmmemory_frames = self.transformer(src, ~mask, query_embed, pos, video_length=video_length,
                                                                                                                  ctxtoken=vidsrc_, gtoken=self.global_rep_token, gpos=self.global_rep_pos, vlen=src_vid_mask.sum(1).long())
        ...
        if vid is not None: ## for demo (run_on_video/run.py)
            ...
            if real_neg_mask.sum() != 0:
                ...
                _, _, memory_neg, memory_global_neg, attn_weights_neg, _, _, _, _ = self.transformer(src_dummy_neg, ~mask_dummy_neg, query_embed, pos_neg, video_length=video_length,
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, eos_coef, losses, temperature, span_loss_type, max_v_l,
        ...
        # foreground and background classification (TODO: Change to the original 0 for foreground for the original softmax-based)
    ...
    def loss_spans(self, outputs, targets, indices):
        ...
        # Added by me to deal with cases where there are no matches
        if len(idx[0]) == 0:
            return losses
    ...
    def loss_labels(self, outputs, targets, indices, log=True):
        ...
        if log:
            ...
            try:
                # Compute the accuracy of the background class
                background_idx = (target_classes == self.background_label).nonzero(as_tuple=True)
                background_accuracy = 100 - accuracy(src_logits[background_idx], self.background_label)[0]
                losses['background_accuracy'] = background_accuracy.item()
            except:
                pass
    ...
    def loss_moment2txt_sim_distill(self, outputs, targets, indices, log=True):
        if outputs["moment2txt_similarity"] is not None:
            ...
            if moment_mask.sum() != 0:
                ...
                # Added by me
                # This is for the case that there are no positive moments
                loss_distill = 0.
        else:
            loss_distill = 0.
    ...
    def forward(self, outputs, targets):
        ...
        if self.use_matcher:
            indices_main = self.matcher(outputs_without_aux, targets)
        ...
        for loss in losses_target:
            losses.update(self.get_loss(loss, outputs, targets, indices_main))
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in losses_target:
                    if loss in ["saliency", "ms_align", "distill", "orthogonal_dummy"]:
                        continue
...
def build_model(args):
    ...
    # Select the matcher that we want (Added by me)
    ...
    losses = ['spans', 'labels', 'ms_align', 'distill', 'orthogonal_dummy', 'saliency']
