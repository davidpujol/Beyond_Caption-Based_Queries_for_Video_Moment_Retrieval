#!/bin/bash
# Master inference script for CG-DETR with raw InternVideo features
...
echo "=========================================="
echo "Running CG-DETR inference on ${DATASET}"
echo "=========================================="
echo ""

# Get the project root and the CGDETR root
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
CGDETR_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

# Set PYTHONPATH to include project root (for 'baselines' module imports)
export PYTHONPATH="${PROJECT_ROOT}:${PYTHONPATH}"

# Change to CGDETR root directory (where cg_detr module is)
cd "${CGDETR_ROOT}"
