...
def merge_intervals(intervals):
    ...
    Args:
        intervals (List[List[float]]): List of intervals [start, end].
    
    Returns:
        List[List[float]]: List of merged non-overlapping intervals.
    """
    if not intervals:
        return []
...
def merge_gt_data(gt_data):
    ...
    for clip_gt in gt_data:
        ...
        original_qid = clip_gt['original_qid']  # If the original_qid is not defined in the dataset, then this is just the same as qid
        vid = clip_gt['vid']
...
def eval_epoch_post_processing(submission, opt, gt_data,
                               save_submission_filename, cuts_type):
    # IOU_THDS = (0.5, 0.7)
    ...
    if opt.eval_split_name in ["val"]:  # since test_public has no GT
        ...
        save_metrics_path = submission_path_no_suffix.replace(".jsonl", "_metrics" + opt.suffix + ".jsonl")    # Add also our suffix
        save_json(metrics,
                  save_metrics_path,
                  save_pretty=True,
                  sort_keys=False)
...
def compute_mr_results(model,
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        for idx, (meta, spans, score) in enumerate(
            ...
            vid = meta['vid'] if 'vid' in meta else meta['video_id']
            cur_query_pred = dict(
                qid=meta["qid"],
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores with sliding windows"):
        ...
        for idx, (meta, spans, score) in enumerate(zip(query_meta, pred_spans.cpu(), scores.cpu())):
            ...
            if opt.span_loss_type == "l1":
                ...
                # Adjust spans by clip start offset
...
def setup_model(opt):
    ...
    if opt.resume is not None:
        ...
        if False and "pt" in opt.resume[:-4]:
