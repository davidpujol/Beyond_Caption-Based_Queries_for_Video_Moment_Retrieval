...
def train(model, criterion, optimizer, lr_scheduler, train_dataset,
    ...
    if opt.a_feat_dir is None:
        train_loader = DataLoader(
            ...
            drop_last=True  # Added by me cause it seems to be failing in some cases if the batch is not full
        )
    else:
        train_loader = DataLoader(
            train_dataset,
...
def start_training():
    ...
    if opt.a_feat_dir is None:
        ...
        if opt.dset_name in ['hl', 'synopground']:
            dataset_config["cuts_type"] = 'single'  # In this case, we just treat the entire video as a single instance/cut.
        elif opt.dset_name == 'hd_epic':
            dataset_config["cuts_type"] = 'multi'   # The training of HD-EPIC requires multiple cuts per video (as long as they include all the GT spans)
    ...
    if opt.eval_path is not None:
        ...
        dataset_config["q_feat_dir"] = opt.t_feat_dir_eval

        # Optionally we can do the sliding window evaluation
        if opt.dset_name == 'hd_epic':
            ...
            dataset_config["cuts_type"] = 'sliding'   
