...
def compute_temporal_iou_pair(gt_window, pred_window):
    """
    Compute temporal intersection-over-union (IoU) for a single pair of temporal windows.

    Args:
        gt_window: array-like of shape (2,), [start (float), end (float)]
        pred_window: array-like of shape (2,), [start (float), end (float)]

    Returns:
        float: IoU value between 0 and 1 for the pair.
    """
    start_inter = max(pred_window[0], gt_window[0])
    end_inter = min(pred_window[1], gt_window[1])
    intersection = max(0.0, end_inter - start_inter)

    start_union = min(pred_window[0], gt_window[0])
    end_union = max(pred_window[1], gt_window[1])
    union = max(0.0, end_union - start_union)
...
# ----------------------------------------------
# ADDED BY ME TO COMPUTE THE MULTIPLICITY-AWARE MAP
# One very important consideration here is that we are considering just one single GT at the time.
# What we want to avoid is that if the original matched version has 3 GT, and now we only consider one of them, 
# we should not be penalizing if the model predicts a hit in position 2. This may not be bad. The first position might be a hit but for another GT.
def compute_multiplicity_aware_ap(ground_truth, prediction, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    ...
    prediction = sorted(prediction, key=lambda x: -x['score'])

    # Prepare ground truth indexing by video for efficient access
    gt_by_video = {}
    for idx, gt in enumerate(ground_truth):
        num_gts_merged = gt['num_gt_merged_window'] # Assuming all GT have the same num_gt_merged_window
        gt['index'] = idx
        gt_by_video.setdefault(gt['video-id'], []).append(gt)

    # Length of compressed TP and FP arrays
    comp_len = max(num_preds - num_gts_merged + 1, 1)

    for t_idx, tiou_threshold in enumerate(tiou_thresholds):
        lock_gt = np.ones(num_gts) * -1  # matched GT indices; -1 = unmatched

        # Initialize compressed arrays
        comp_tp = np.zeros(comp_len)
        comp_fp = np.zeros(comp_len)

        # For predictions ranked beyond n_gt (positions start at 1 in comp arrays)
        # map original pred idx to compressed idx = pred_idx - n_gt + 1

        for pred_idx, pred in enumerate(prediction):
            video_gts = gt_by_video.get(pred['video-id'], [])
            matched_gt_idx = -1
            best_iou = 0
            ...
            for gt in video_gts:
                ...
                if lock_gt[gt_idx] != -1:
                    continue
                pred_box = np.array([[pred['t-start'], pred['t-end']]])
                gt_box = np.array([[gt['t-start'], gt['t-end']]])
                iou = compute_temporal_iou_batch_cross(pred_box, gt_box)[0]
                if iou >= tiou_threshold and iou > best_iou:
                    matched_gt_idx = gt_idx
                    best_iou = iou

            if matched_gt_idx != -1:
                lock_gt[matched_gt_idx] = pred_idx
                if pred_idx < num_gts_merged:
                    # Mark hit in compressed TP position 0
                    comp_tp[0] = 1
                else:
                    # Mark hit in compressed TP position pred_idx - num_gts + 1
                    comp_idx = pred_idx - num_gts_merged + 1
                    comp_tp[comp_idx] = 1
            else:
                # False positive
                if pred_idx < num_gts_merged:
                    # Record false positives inside top predictions in compressed position 0
                    # We do not add FP multiple times; handled as rare approx.
                    # Alternatively, ignore to avoid penalizing noise inside the hit bucket
                    pass
                else:
                    comp_idx = pred_idx - num_gts_merged + 1
                    comp_fp[comp_idx] = 1

        tp_cumsum = np.cumsum(comp_tp)
        fp_cumsum = np.cumsum(comp_fp)
        recalls = tp_cumsum / num_gts
        precisions = tp_cumsum / (tp_cumsum + fp_cumsum + 1e-10)

        ap[t_idx] = interpolated_precision_recall(precisions, recalls)

    return ap


def compute_window_multiplicity_aware_ap_old(
    ground_truth,
    prediction,
    ...
    - Each prediction is assigned at most once to the GT with highest IoU above threshold.
    - Earlier FPs remain; earlier TPs for other GTs do not count for this GT.
    """

    num_thresholds = len(tiou_thresholds)
    num_gts = len(ground_truth)
    ap = np.zeros(num_thresholds)
    if len(prediction) == 0 or num_gts == 0:
        return ap, num_gts

    # Sort predictions by decreasing score
    prediction = sorted(prediction, key=lambda x: -x["score"])

    # Organize GTs by video
    gts_by_vid = {}
    for i, gt in enumerate(ground_truth):
        gt["index"] = i
        gts_by_vid.setdefault(gt["video-id"], []).append(gt)

    for vid, gts in gts_by_vid.items():
        preds = [p for p in prediction if p["video-id"] == vid]
        if not preds:
            continue

        # Precompute IoU matrix: shape (n_pred, n_gt)
        _pred = np.array([[p["t-start"], p["t-end"]] for p in preds])
        _gt = np.array([[g["t-start"], g["t-end"]] for g in gts])
        ious_all = compute_temporal_iou_batch_cross(_pred, _gt)[0]

        n_gt = len(_gt)
        
        for t_idx, tiou_threshold in enumerate(tiou_thresholds):
            matched_gts = set()  # GTs already assigned
            matched_preds = set()

            # Iterate over predictions in score order
            for idx, pred in enumerate(preds):
                # Find IoUs with GTs that are still available
                candidate_idxs = [gt_idx for gt_idx, gt in enumerate(gts) if gt_idx not in matched_gts and ious_all[idx, gt_idx] >= tiou_threshold]

                if candidate_idxs:
                    # Assign to the GT with the highest IoU
                    best_gt_idx = candidate_idxs[np.argmax(ious_all[idx, candidate_idxs])]
                    matched_gts.add(best_gt_idx)
                    matched_preds.add(idx)
                    
                    tp, fp = [], []
                    # Iterate over all the previous predictions
                    for p_idx in range(idx+1):
                        if p_idx == idx:
                            # This is the current TP
                            tp.append(1)
                            fp.append(0)
                        else:
                            # Ignore a prediction if it was matched to one
                    ...
                    if tp:
                        ...
                        ap[t_idx] += interpolated_precision_recall( np.array(precision), np.array(recall))

    return ap, num_gts



def compute_window_multiplicity_aware_ap(
    ground_truth,
    prediction,
    tiou_thresholds=np.linspace(0.5, 0.95, 10),
):
    ...
    for vid, gts in gts_by_vid.items():
        ...
        _pred = np.array([[p["t-start"], p["t-end"]] for p in preds])
        _gt = np.array([[g["t-start"], g["t-end"]] for g in gts])
        segs_to_eval = np.array([[gt['se']] for gt in gts])
        ...
        for t_idx, tiou_threshold in enumerate(tiou_thresholds):
            ...
            for idx, pred in enumerate(preds):
                ...
                if candidate_idxs:
                    ...
                    tp, fp = [], []
                    # Iterate over all the previous predictions (so here is where we are considering all the previous)
                    # Note that in this metric we are already implicitely dealing with the problem of repeated matches of predictions to the same gt
                    # Basically since we enforce that a prediction can only match a previously unmatched GT.
                    for p_idx in range(idx+1):
                        if p_idx == idx:
                            # This is the current TP
                            tp.append(1)
                    ...
                    if tp:
                        ...
                        # Only include this computation if the corresponding matched GT should be evaluated
                        if segs_to_eval[best_gt_idx]:
                            ap[t_idx] += interpolated_precision_recall( np.array(precision), np.array(recall))
...
def compute_average_precision_detection(ground_truth,
    ...
    for idx, pred in enumerate(prediction):
        ...
        #multiplicity = len(_gt)
        multiplicity = sum(segs_to_eval) 
