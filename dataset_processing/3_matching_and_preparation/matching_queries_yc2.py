...
def stage1_cluster_and_merge(input_file: str, temp_file: str, similarity_threshold: float = 0.85):
    ...
    # Importantly, the original annotations do not include a qid, which we need to keep track of the queries
    # We can thus simply add the index to them
    ...
    with jsonlines.open(temp_file, mode='w') as writer:
        for video_id, anns in video_groups.items():
            ...
            for cluster in clusters:
                ...
                intervals = [a['relevant_windows'] for a in cluster_anns]
                ...
                writer.write({
                    "video_id": video_id,
                    "qid": cluster_anns[0]['qid'],  # Just choose one of the original IDs
                    "narration": cluster_anns[0]['narration'],  # Just choose one of the original narrations
                    "query_underspecified": placeholder_narration,  # Placeholder, will be updated in stage 2
                    "relevant_windows": intervals,
                    "unique_narration_id": cluster_anns[0]['qid'],  # Just choose one of the original IDs
                    "merged_from": [a['qid'] for a in cluster_anns],
                    "original_queries": original_queries,    # Keep track of all the original queries that we merged
                    "duration": cluster_anns[0]['duration'] if 'duration' in cluster_anns[0] else None,
                    "recipe_type": cluster_anns[0]['recipe_type'] if 'recipe_type' in cluster_anns[0] else None,
                    "subset": cluster_anns[0]['subset'] if 'subset' in cluster_anns[0] else None
                })
        ...
        for ann in reader:
            if len(ann['merged_from']) > 1:
                ...
                if use_gemma:
                    ...
                    prompt = f"""
                        Combine these queries into a single general unified description that applies to all the queries. Avoid adding unncessary ands/ors, trying to make it as concise as possible:
                        Example 1:
                        Queries: ["Prepare vegetables.", "Prepare food.", "Cook vegetables."]
                        Unified description: "Prepare food."   
                        Example 2:
                        Queries: ["Season chicken.", "Season the chicken.", "Cook poultry."]
                        Unified description: "Cook meat."
