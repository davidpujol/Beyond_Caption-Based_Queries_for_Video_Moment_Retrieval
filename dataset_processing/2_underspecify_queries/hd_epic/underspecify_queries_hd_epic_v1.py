...
def simplify_query(query: str) -> str:
    prompt = f"""Simplify this query by removing or generalizing unnecessary attributes and modifiers (adjectives and adverbs), but keeping the sentence’s meaning.  
    ...
    Simplified query: Throw the orange into the bag using the hand. With the hand, pick up the roll and pull the roll to pull a bag using the hand.

    Example 2:
    Query: Open the upper cupboard by holding the handle of the cupboard with the left hand.
    Simplified query: Open the cupboard by holding the handle with the hand.
    ...
    Simplified query: Using the hand, remove the cover of the scissors.
...
def main():
    ...
    output_file = output_dir / (input_path.stem + "_underspecified_attributes_modifiers_only_queries_llm.jsonl")
...
# Usage HD-EPIC:
# python dataset_processing/datasets_augmentation_generation/underspecified_queries/hd_epic/underspecify_queries_hd_attributes_modifiers_hd_epic.py ${DATA_ROOT}/HD_EPIC/HD_EPIC_Narrations.pkl ${DATA_ROOT}/HD_EPIC/underspecified/attribute_modifiers_simplification